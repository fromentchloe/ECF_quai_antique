...
// Requête SQL pour insérer un nouvel utilisateur dans la base de données
            $stmt = mysqli_prepare($conn, "INSERT INTO users (name, email, password, is_admin) VALUES (?, ?, ?, ?)");
            mysqli_stmt_bind_param($stmt, "sssi", $name, $email, $password_hash, $is_admin);
...
// L'inscription a réussi,stocker le nom de l'utilisateur dans la session
            ...
            if ($is_admin == 1) {
                ...
                echo '<div><h2>Nous sommes heureux de vous compter parmi nos clients.</h2><br><a style="background-color: #7f5539; color: #e6ccb2; border-radius: 20px; padding: 10px 20px; font-size: 25px; cursor: pointer; text-decoration:none" href="javascript:history.go(-1)>Retour à l\'accueil</a></div>';
            }
